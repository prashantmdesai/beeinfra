// =============================================================================
// DATS-BEEUX-INFR-DEV VM3 - VIRTUAL MACHINE MODULE (KUBERNETES MASTER)
// =============================================================================
// Creates the dats-beeux-infr-dev Ubuntu VM for Kubernetes master with fresh installation
// Same hardware specs as data VM but fresh OS installation for clean Kubernetes setup
// =============================================================================
...
@description('VM size - Same as data VM')
    ...
    ipConfigurations: [
      {
        ...
        properties: {
          ...
          privateIPAddress: '10.0.1.6'  // Static IP for Kubernetes master
          publicIPAddress: {
            id: publicIp.id
          }
          subnet: {
  ...
  properties: {
    ...
    storageProfile: {
      ...
      osDisk: {
        ...
        deleteOption: 'Delete'
        diskSizeGB: 30  // Same as data VM
        managedDisk: {
          storageAccountType: 'Premium_LRS'  // Same as data VM
        }
        osType: 'Linux'
      }
...
// Custom Script Extension for comprehensive VM3 setup (software + Azure File Share + networking)
...
@description('Kubernetes API Server Internal URL')
output kubernetesApiInternal string = 'https://${networkInterface.properties.ipConfigurations[0].properties.privateIPAddress}:6443'

@description('Kubernetes API Server External URL')
output kubernetesApiExternal string = 'https://${publicIp.properties.ipAddress}:6443'
...
@description('VM Role')
output vmRole string = 'KubernetesMaster'
